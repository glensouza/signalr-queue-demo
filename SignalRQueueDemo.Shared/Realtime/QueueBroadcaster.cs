...
/// broadcast identically. Group/target filtering when CORS + the trust boundary land, switching to the Azure
/// SignalR path, or adding logging happens here once instead of in three handlers where one could be missed.
/// </para>
///
/// <para>
/// <b>A broadcast can never fail an already-committed mutation.</b> The write is durable before we get here, so
/// if the push throws (a faulted SignalR pipeline today, or a network send to the Azure SignalR service once
/// that path is enabled) we log and swallow rather than let it propagate. Letting it bubble would turn a successful check-in
