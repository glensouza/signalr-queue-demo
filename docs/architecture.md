...
**Why `KnownNetworkIdentifiers.LocalhostNetwork` is explicit, not the default `GetEndpoint(name)` call:** an endpoint reference with no network context resolves *relative to whichever resource consumes it*. Consumed by a container (all three Angular resources), the unqualified call resolves to Aspire's internal container-network tunnel address (`http://aspire.dev.internal:{port}`) — reachable from inside Aspire's Docker network, not from an actual browser. Both `API_BASE_URL` (read by browser JS) and the CORS origins below (matched against the browser's `Origin` header) need the one address a real browser on the host machine can reach, regardless of which Aspire resource is the technical "consumer" of the reference — hence forcing `LocalhostNetwork` explicitly in both directions. See the dated entry in `docs/decisions.md` for how this was found (by testing against a live `aspire run`, not by reading the API alone) and a related trap: the resolved `http://localhost:{port}` is Aspire's own DCP proxy port, not the port `docker ps` shows for that container — the two differ, and only the DCP-proxy port is what every Aspire-aware reference (and a real browser) actually uses.
...
Restricted CORS (`Cors:AllowedOrigins`, policy `KnownFrontends`) is applied across **both** zones — every browser-reachable surface: the public and staff REST endpoints and the SignalR hub. It is not itself the trust boundary (the auth rows above are); it just keeps the legitimate cross-origin frontends — public *and* staff Angular apps, which all reach the hub for live updates — from being refused by the browser before those checks run, while an unlisted origin still is. Once containerized, each Angular app's allowed origin is no longer the fixed dev-server placeholder in `appsettings.json` — `AppHost.cs` overrides `Cors:AllowedOrigins[0..2]` at startup with each container's actual Aspire-assigned address (see [Containerizing the Angular apps](#containerizing-the-angular-apps) above). See `docs/decisions.md`, "CORS allowed origins are config" and the follow-up entry on the `KnownNetworkIdentifiers.LocalhostNetwork` fix.
