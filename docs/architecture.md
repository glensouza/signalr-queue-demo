...
- **Angular `queue-display`** advertises the **public-checkin Angular app** (a separate app it can't share code with). That URL is injected into only the `queue-display` container as `PUBLIC_CHECKIN_URL` (from public-checkin's endpoint, `LocalhostNetwork`-pinned like `API_BASE_URL`) and surfaced through the optional `RuntimeConfig.publicCheckinUrl`. The QR is encoded **server-side** by the API's `GET /checkin/qr` endpoint (shared `QrCodeHelper.GenerateSvg`, from the `PublicCheckinUrl` the AppHost injects into the API); the board simply points an `<img src>` at that endpoint, so it carries no client-side QR library.
