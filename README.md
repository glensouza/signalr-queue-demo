...
| `POST /queue/{id}/cancel` | public (hardened) | Moves a `Waiting`/`Serving` entry to `Cancelled` — the kiosk "Stop tracking" action. Public write, so it carries the same check-in token as `POST /checkin`, not staff auth. |
...
`/checkin`, `/staff`, `/display` — the same three experiences the Angular apps provide, **fully self-encapsulated
with no runtime dependency on the API process at all**: check-in/call-next/complete call directly into
`SignalRQueueDemo.Shared`'s `IQueueRepository` (also referenced by `ApiService`) rather than round-tripping to a
sibling process, and Blazor hosts its **own** SignalR `QueueHub` — the same hub class, which now lives in
...
Two things worth flagging for the vendor team (see `docs/decisions.md` for the full entries): because Blazor hosts
its own hub, `QueueRealtimeService` connects to its **own** base address (`NavigationManager.BaseUri` +
`/hubs/queue`) — a same-process loopback connection — rather than resolving `ApiService`'s endpoint. And on
storage, the two stacks share the Azurite emulator (tables/blobs, when `Persistence:Provider=TableStorage`) but
...
Each of the three Angular apps is containerized via `builder.AddDockerfile(name, "../SignalRQueueDemo.Angular", "Dockerfile").WithBuildArg("APP_NAME", ...)` — one parameterized `Dockerfile` (see [Containers](#containers-the-angular-apps) below), three resources differing only in which app the build arg selects. Each container gets `API_BASE_URL` injected from `apiService.GetEndpoint("http", KnownNetworkIdentifiers.LocalhostNetwork)` — explicitly pinned to `LocalhostNetwork` because the *unqualified* `GetEndpoint(name)` resolves relative to the consuming resource, and for a container consumer that turned out to be Aspire's internal container-network tunnel address (`http://aspire.dev.internal:{port}`), unreachable from an actual browser. CORS is deliberately **not** wired from the AppHost: Aspire serves each app under a per-run `*.dev.localhost` hostname the AppHost can't name ahead of time (an early attempt to inject those origins pinned the host to `localhost` and silently failed CORS in a real browser), so the API instead accepts any loopback-family origin via `Cors:AllowLoopbackOrigins` (default `true`) — see the [Security model](#security-model) section and the 2026-07-12 entry in `docs/decisions.md`. All of this is verified against a live `aspire run`.

#### Containers: the Angular apps

`SignalRQueueDemo.Angular/Dockerfile` is one multi-stage build (`node:22-slim` → `nginx:alpine`) parameterized by an `APP_NAME` build arg, shared by all three apps — see the file's own header comment for why one Dockerfile replaces three near-duplicates and why the build context is the whole Angular workspace folder, not a per-app subfolder. `SignalRQueueDemo.Angular/docker/write-runtime-config.sh` runs as an nginx `/docker-entrypoint.d/` script at container start, overwriting `config.json` from the `API_BASE_URL` environment variable Aspire injects — never baked in at `docker build` time, since the API's address isn't known until `aspire run` assigns it. All three containers get the identical `config.json` (just `apiBaseUrl`): the queue-display board's check-in QR + URL are fetched from the API (`GET /checkin/qr`, `GET /checkin/url`) rather than injected into its container, so the API is the single source of the public-checkin address. `SignalRQueueDemo.Angular/docker/nginx.conf` serves the SPA (`try_files ... /index.html` for client-side routing) and marks `config.json` `no-store` so a container restart's new address is never masked by a cached copy of the old one.
...
| `SignalRQueueDemo.ApiService` | Minimal API: `GET /checkin/token`, `/checkin`, `/queue/call-next`, `/queue/{id}/complete`, `GET /queue`, `GET /queue/since/{sequenceNumber}`, backed by `IQueueRepository` → `SqliteQueueRepository` or `TableStorageQueueRepository` (config-selected); plus `POST /checkin/{id}/documents`, `GET /queue/{id}/documents`, `GET /queue/{id}/documents/{docId}`, backed by `IDocumentRepository` (same config-selected backend) and `DocumentBlobStore` (Azurite Blob Storage, not config-selected); staff routes (`call-next`, `complete`, `GET /staff/verify` — the sign-in key check — and document viewing) gated by `StaffAuthFilter`, the check-in path by a short-lived check-in token, all browser surfaces (REST + hub) behind the `KnownFrontends` CORS policy (see [Security model](#security-model)); plus a self-hosted `QueueHub` (`/hubs/queue`, the class shared from `SignalRQueueDemo.Shared.Realtime`) broadcasting `QueueUpdated` to the Angular clients, and the `UseAzureSignalR` escape hatch (`AzureSignalRDefaultModeStub`, `AzureSignalRServerlessDemoService`). The Blazor app hosts its own separate instance of the same hub for its own circuits. |
...
**Restricted CORS** (policy `KnownFrontends`) — applied to **every browser-reachable surface**: the public *and* staff REST endpoints and the SignalR hub. CORS is not the trust boundary (the staff key and check-in token are) — it's applied everywhere, including staff endpoints and the hub, precisely so the legitimate cross-origin frontends (all three Angular apps depend on the hub for live updates; `internal-queue` calls the staff endpoints) aren't refused by the browser before those real checks run, while an unlisted origin still is. The policy accepts an origin if it's listed explicitly in `Cors:AllowedOrigins` (seeded with the local dev-server ports `http://localhost:4200`–`4202`, for running the apps outside containers) **or**, when `Cors:AllowLoopbackOrigins` is `true` (the POC default), if its host is loopback — `localhost`, `127.0.0.1`, `::1`, or any `*.localhost`. The loopback allowance is what lets the containerized apps work, because Aspire serves each under a per-run `*.dev.localhost` hostname no fixed list can predict; it's safe only because this POC runs on a single, network-isolated machine where every such host is loopback. **For any deployment reachable beyond localhost, set `Cors:AllowLoopbackOrigins=false` and list the real origins.** Missing `StaffAuth:Key` or `CheckInToken:SigningKey` fails the app at **startup**, not on the first request.
...
3. From the `.http` file (or a second tool), run `POST /queue/call-next` repeatedly. Watch the kiosk's position count down **live** over SignalR — no refresh. When this visitor's own entry is called, the screen switches to "It's your turn"; when it's completed (`POST /queue/{id}/complete`), that person's card shows "Thank you" and auto-removes after a short visible countdown.
4. **Check in someone else (multi-tracking):** tap **Check someone else in**. The form reappears *without* discarding the first person — check in a second visitor and the device now shows **both** cards, each with its own live position. This is deliberate: checking someone else in never costs you sight of your own spot. Tap **Stop tracking** on a card to drop just that person — this also cancels their entry on the backend (`POST /queue/{id}/cancel`), so they disappear from the staff console and board live, not just from this device.
5. **Document upload:** before completion, use "Choose file" on a person's card to attach a PDF/JPEG/PNG (≤10 MB). A wrong type or oversized file is rejected instantly client-side (the same rules the server enforces — see [Uploading and viewing documents](#uploading-and-viewing-documents)); a valid file uploads, and the staff console's "View documents" control for that entry appears **live**. Confirm it landed with `GET /queue/{entryId}/documents` (staff-gated).
6. **Reconnect/catch-up (acceptance criterion):** while on the position screen, open DevTools → Network → set **Offline**. Run a `POST /queue/call-next` or two against the API. Set the network back to **Online** — the kiosk reconnects and the position corrects itself to the changes it missed while offline (a brief "Reconnecting… your place is still saved" note appears while it's degraded). This exercises the shared `QueueHubService`'s catch-up path end-to-end from a real browser.
...
1. Open `http://localhost:4201`. The staff sign-in form asks for a staff key. Enter the `StaffAuth:Key` value from `SignalRQueueDemo.ApiService/appsettings.json` (the same value the `@staffKey` variable in `SignalRQueueDemo.ApiService/SignalRQueueDemo.ApiService.http` uses) — currently the placeholder `PLACEHOLDER-staff-key-not-a-real-secret-override-via-user-secrets`. The form itself shows a hint pointing at that config key. Sign-in verifies the key against the API (`GET /staff/verify`) before proceeding, so a wrong key is rejected right here rather than accepted and failing on a later action.
...
5. **View documents** only appears on an entry that actually has documents (with the count, e.g. "View documents (2)") — attach one via `POST /checkin/{entryId}/documents` (or the public-checkin upload) and watch the control appear **live** on the console. Click it to view inline: images in an `<img>`, PDFs in an `<iframe>` with the browser's native viewer. **Object URL lifecycle:** selecting a different document revokes the previous one; changing entries does the same; closing the panel revokes any open URL. When you **Complete** an entry, its documents are deleted (content lives only while the visitor is in the queue), so the control is gone if you look again. Note the **Call Next** button sits at the top of the Waiting column, under the waiting count, so it's reachable without scrolling past a long list.
...
7. **Wrong staff key:** refresh to return to sign-in and enter a wrong key. Sign-in verifies it against `GET /staff/verify` and rejects it inline ("That staff key is not valid") without ever leaving the sign-in screen. Enter the correct key and the console loads. (As defence in depth the console still handles a `401` on its own actions — bouncing to sign-in on a Call Next / Complete, an inline "sign in again" on a document view — which now only happens if a valid key is rotated server-side mid-session.)
...
1. Open `http://localhost:4202`. The board initially shows "Now Serving" (empty initially) on the left and "Waiting" with the seeded entries (shown as `A-042` "Jane T.", `A-043` "Sam S.") on the right, sorted by check-in time, plus a check-in QR/URL panel.
...
- **The reconnect/catch-up plumbing delta.** This is the single biggest difference the two stacks exposed. Angular's `QueueHubService` talks to a **process it doesn't share code with** — every question ("what did I miss", "what's current state", "record this mutation") is an HTTP call, so it needs a full REST client, and reconnect is a network problem "solved once" for browser code that can't see the server's internals. Blazor's `QueueRealtimeService` runs **in the same process as a full repository handle** — catch-up and the polling fallback are the exact same in-process method calls `ApiService`'s own REST handlers make, no HTTP round-trip, no separate client to keep in sync. Blazor now also hosts its **own** `QueueHub` (the same class `ApiService` hosts, shared from `SignalRQueueDemo.Shared.Realtime`), so it has no runtime dependency on the API at all; `NotifyMutation` fans a local write out to the app's other Blazor circuits over a loopback `HubConnection`. The tradeoff this makes concrete: the two stacks are now genuinely independent — the price of full self-encapsulation is that live updates don't cross the stack boundary (and, under SQLite, neither does data). Direct repository access still made the client noticeably simpler to write; each stack just owns its whole runtime now.
...
- **Service discovery.** Now a non-issue for Blazor: because it hosts its own hub and reads/writes its own store, `webfrontend` has no `WithReference(apiService)` at all — nothing to resolve. (An earlier iteration did connect Blazor's `HubConnection` to `ApiService`'s hub and hit a real snag worth remembering: SignalR's WebSocket transport opens a raw socket that bypasses Aspire's service-discovery-aware `HttpMessageHandler`, so `http://apiservice` didn't resolve and the concrete endpoint had to be read from config. Self-hosting the hub sidesteps that entirely.) The Angular containers resolve the API's external, browser-reachable address directly, as before.
