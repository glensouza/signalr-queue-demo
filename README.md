...
3. **`/display`:** read-only board, ticket + masked name (e.g. "Jane T."), no `servedBy`, Completed entries excluded. Confirm it updates live as `/staff` calls next/completes. It also shows the same **check in from your phone** QR + URL as the Angular board (`CheckInQr.razor`), encoded in-process via the shared `QrCodeHelper` rather than fetched from the API.
