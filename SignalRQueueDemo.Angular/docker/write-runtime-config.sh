...
# Hand-built JSON, not a templating tool: this is the one field RuntimeConfig (runtime-config.ts) declares, and
# API_BASE_URL is an Aspire-supplied endpoint URL (scheme://host:port), never client input, so there's no
# untrusted content here needing escaping.
